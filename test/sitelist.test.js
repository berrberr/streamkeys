...
describe("sitelist", function () {
  ...
  it("matches valid site urls", function () {
    expect(sitelist.checkMusicSite("http://" + siteNames[0] + ".com")).toBe(
      true
    );
    expect(sitelist.checkMusicSite("http://www." + siteNames[1] + ".fm")).toBe(
      true
    );
    expect(
      sitelist.checkMusicSite("http://" + siteNames[2] + ".someothertld")
    ).toBe(true);
    expect(
      sitelist.checkMusicSite("http://music." + siteNames[3] + ".com")
    ).toBe(true);
  });
  ...
  it("doesn't match invalid site urls", function () {
    ...
    // Wrong sitename
    expect(
      sitelist.checkMusicSite("http://" + siteNames[0] + "somenonesense.com")
    ).toBe(false);
    // Sitename in querystring
    expect(
      sitelist.checkMusicSite(
        "https://www.someothersite.com/index?site=www." + siteNames[0] + ".com"
      )
    ).toBe(false);
  });

  it("gets site name from valid url", function () {
    expect(sitelist.getSitelistName("http://" + siteNames[0] + ".com")).toBe(
      siteNames[0]
    );
    expect(sitelist.getSitelistName("http://" + siteNames[1] + ".com")).toBe(
      siteNames[1]
    );
    expect(sitelist.getSitelistName("http://" + siteNames[2] + ".com")).toBe(
      siteNames[2]
    );
    expect(sitelist.getSitelistName("http://" + siteNames[3] + ".com")).toBe(
      siteNames[3]
    );
  });
  ...
  it("toggles site aliases", function (done) {
    sitelist
      .setSiteState(siteNames[0], { alias: ["anewsitealias"] })
      .then(function () {
        expect(sitelist.getController("http://anewsitealias.com")).toBe(
          controllerNames[0]
        );
        sitelist
          .setSiteState(siteNames[0], {
            alias: [],
            removedAlias: ["anewsitealias"],
          })
          .then(function () {
            expect(sitelist.getController("http://anewsitealias.com")).not.toBe(
              controllerNames[0]
            );
            done();
          });
  ...
  it("matches ip aliases", function (done) {
    sitelist
      .setSiteState(siteNames[0], { alias: ["192.168.0.1", "localhost:3000"] })
      .then(function () {
        expect(sitelist.getController("http://192.168.0.1")).toBe(
          controllerNames[0]
        );
        expect(sitelist.getController("http://localhost")).not.toBe(
          controllerNames[0]
        );
        expect(sitelist.getController("http://localhost:3000")).toBe(
          controllerNames[0]
        );
        done();
      });
  ...
  it("toggles enable/disable of a site", function (done) {
    ...
    sitelist.setSiteState(siteNames[0], { enabled: false }).then(function () {
      ...
      sitelist.getMusicTabs().then(function (musicTabs) {
        ...
        sitelist
          .setSiteState(siteNames[0], { enabled: true })
          .then(function () {
            expect(sitelist.getEnabled().length).toBe(siteNames.length);
...
describe("loading settings from chrome.storage.sync", function () {
  ...
  describe("loading v0 settings", function () {
    ...
    it("creates properly structured storage object", function (done) {
      chrome.storage.sync.get(function (storageObject) {
        ...
        expect(sites[siteNames[0]].alias).toEqual(
          mockedSites[siteNames[0]].alias
        );
        expect(sites[siteNames[1]].alias).toEqual(
          mockedSites[siteNames[1]].alias
        );
  ...
  describe("loading v1 settings with missing and undefined sites", function () {
    ...
    beforeAll(function () {
      ...
      settingObj[siteNames[1]] = {
        enabled: undefined,
        priority: 2,
        alias: undefined,
      };
      settingObj[siteNames[2]] = {
        enabled: true,
        priority: 3,
    ...
    it("sets aliases properly", function () {
      expect(storageSitelist.getSitelistName("http://www.aliasone.com")).toBe(
        siteNames[2]
      );
    });
  });
...
describe("site priority", function () {
  ...
  it("sets priority", function (done) {
    prioritySitelist
      .setSiteState(siteNames[1], { priority: 3 })
      .then(function () {
        expect(prioritySitelist.getPriority(siteNames[1])).toBe(3);
        prioritySitelist
          .setSiteState(siteNames[2], { priority: 3 })
          .then(function () {
  ...
  it("only returns tabs with highest priority", function (done) {
    prioritySitelist.getActiveMusicTabs().then(function (tabs) {
      var activeUrls = tabs.map(function (tab) {
        return tab.url;
      });
  ...
  it("doesn't overwrite settings on priority update", function (done) {
    prioritySitelist
      .setSiteState(siteNames[1], { priority: 5, enabled: false })
      .then(function () {
