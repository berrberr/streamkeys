...
describe("sitelist", function () {
  ...
  beforeAll(function () {
    ...
    sitesWithUrl = _.pickBy(sitelist.sites, function (siteData) {
      return !_.isEmpty(siteData.url);
    });

    sitesMapping = _.map(sitesWithUrl, function (siteData, siteKey) {
      return {
        key: siteKey,
        urls: [siteData.url],
        controller: siteData.controller
          ? siteData.controller
          : siteKey[0].toUpperCase() + siteKey.slice(1) + "Controller.js",
      };
    });

    // Extend sites that have multiple URLs (or different player URLs) here
    _.result(
      _.find(sitesMapping, function (site) {
        return site.key == "play.google";
      }),
      "urls"
    ).push("http://play.google.com/music");
    _.result(
      _.find(sitesMapping, function (site) {
        return site.key == "spotify";
      }),
      "urls"
    ).push("http://play.spotify.com", "http://player.spotify.com");
  });
